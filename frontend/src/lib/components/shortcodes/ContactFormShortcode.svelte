  ...
  <form class="my-6 space-y-4" onsubmit={onSubmit} novalidate>
    {#each form.fields as f (f.name + ':' + f.type)}
      ...
      {:else if f.type === 'submit'}
        ...
        >
          {submitting ? 'Sending…' : (f.label || 'Send')}
        </button>
      {:else}
        <div class="space-y-1.5">
          <label class="block text-sm font-medium text-gray-700" for={'cf-' + f.name}>
            {fieldLabel(f)}
            {#if f.required}<span class="ml-0.5 text-red-500">*</span>{/if}
          </label>
          ...
          {:else if f.type === 'radio'}
            <div class="space-y-1.5" role="radiogroup" aria-labelledby={'cf-' + f.name + '-label'}>
              {#each f.options ?? [] as opt (opt.value)}
