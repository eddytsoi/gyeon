<script lang="ts">
  ...
  import { adminReorderPostCategories } from '$lib/api/admin';
  ...
  let editing = $state<PostCategory | null>(null);
  let deleteTarget = $state<PostCategory | null>(null);
...
<svelte:head><title>{m.admin_cms_post_categories_title()}</title></svelte:head>
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
      ...
      <form method="POST" action={editing ? '?/update' : '?/create'}
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">{m.admin_cms_post_categories_label_desktop_banner()}</label>
          <MediaPicker
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">{m.admin_cms_post_categories_label_mobile_banner()}</label>
          <MediaPicker
            files={data.mediaFiles ?? []}
            value={fMobileBanner}
