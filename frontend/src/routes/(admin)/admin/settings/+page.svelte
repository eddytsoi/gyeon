<script lang="ts">
  ...
  const TOGGLE_KEYS = new Set(['maintenance_mode', 'mcp_enabled', 'blog_enabled', 'pwa_enabled', 'pdp_taobao_layout_enabled']);
  ...
  const FREE_SHIPPING_KEYS = new Set([
  ...
  const SETTING_DESCS = $derived<Record<string, string>>({
    ...
    pwa_enabled: m.admin_settings_desc_pwa_enabled(),
  ...
  const pwaSetting = $derived(data.settings.find((s) => s.key === 'pwa_enabled'));
  let pwaOn = $state(pwaSetting?.value !== 'false');
...
<div class="max-w-3xl">
  ...
  <form method="POST" action="?/save"
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-4">
      <div class="flex flex-col gap-0.5">
        <p class="text-sm font-semibold text-gray-900">{m.admin_settings_label_site_name()}</p>
        <p class="text-xs text-gray-400">{m.admin_settings_desc_site_name()}</p>
      </div>
      <div class="mt-5 flex flex-col gap-1.5">
    ...
    <!-- Checkout page layout — switches the storefront checkout between the
         classic multi-section page and the new modern stepped layout. -->
    <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-4">
      <div class="mb-5">
        <h2 class="text-sm font-semibold text-gray-900">{m.admin_settings_label_checkout_page_layout()}</h2>
        <p class="text-xs text-gray-400 mt-0.5">{m.admin_settings_checkout_page_layout_subtitle()}</p>
      </div>
      <div class="flex flex-col gap-1.5">
        <select id="checkout_page_layout" name="checkout_page_layout" bind:value={checkoutPageLayout}
                aria-label={m.admin_settings_label_checkout_page_layout()}
                class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white
                       focus:outline-none focus:ring-2 focus:ring-gray-900">
          <option value="classic">{m.admin_settings_checkout_page_layout_option_classic()}</option>
          <option value="modern">{m.admin_settings_checkout_page_layout_option_modern()}</option>
        </select>
        <p class="text-xs text-gray-400 mt-0.5">
    ...
    {#if installerFreeShippingSetting}
      <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-4">
        <div class="flex items-start justify-between gap-4 mb-5">
          <div>
            <h2 class="text-sm font-semibold text-gray-900">{m.admin_settings_label_free_shipping_threshold_installer_enabled()}</h2>
            <p class="text-xs text-gray-400 mt-0.5">{m.admin_settings_free_shipping_threshold_installer_subtitle()}</p>
          </div>
          <button type="button"
                  onclick={() => (installerFreeShippingEnabled = !installerFreeShippingEnabled)}
                  class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent
                         transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2
                         {installerFreeShippingEnabled ? 'bg-green-500' : 'bg-gray-200'}"
                  role="switch"
                  aria-checked={installerFreeShippingEnabled}>
            <span class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform
                         transition duration-200 {installerFreeShippingEnabled ? 'translate-x-5' : 'translate-x-0'}"></span>
          </button>
          <input type="hidden" name="free_shipping_threshold_installer_enabled" value={installerFreeShippingEnabled ? 'true' : 'false'} />
        </div>

        <div class="flex flex-col gap-1.5 {installerFreeShippingEnabled ? '' : 'opacity-50'}">
          <label for="free_shipping_threshold_installer_hkd" class="text-xs font-semibold text-gray-500 uppercase tracking-wide">
            {m.admin_settings_label_free_shipping_threshold_installer_hkd()}
          </label>
          {#if SETTING_DESCS['free_shipping_threshold_installer_hkd'] ?? installerFreeShippingSetting.description}
            <p class="text-xs text-gray-400 -mt-0.5">{SETTING_DESCS['free_shipping_threshold_installer_hkd'] ?? installerFreeShippingSetting.description}</p>
          {/if}
          <input id="free_shipping_threshold_installer_hkd" name="free_shipping_threshold_installer_hkd" type="text"
                 value={installerFreeShippingSetting.value}
                 disabled={!installerFreeShippingEnabled}
                 class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-mono
                        focus:outline-none focus:ring-2 focus:ring-gray-900 disabled:bg-gray-50 disabled:cursor-not-allowed" />
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-4">
      <h2 class="text-sm font-semibold text-gray-900 mb-5">
        {m.admin_settings_email_rate_limit_heading()}
      </h2>
      <div class="flex flex-col gap-4">
        <div class="flex flex-col gap-1.5">
          <label for="email_daily_limit"
                 class="text-xs font-semibold text-gray-500 uppercase tracking-wide">
            {m.admin_settings_email_daily_limit()}
          </label>
          <p class="text-xs text-gray-400 -mt-0.5">
            {m.admin_settings_email_daily_limit_hint()}
          </p>
          <input id="email_daily_limit" name="email_daily_limit"
                 type="number" min="0"
                 value={settingValue('email_daily_limit') || '450'}
                 class="w-32 border border-gray-200 rounded-xl px-3 py-2.5 text-sm
                        focus:outline-none focus:ring-2 focus:ring-gray-900" />
        </div>
        <div class="flex flex-col gap-1.5">
          <label for="email_rate_per_minute"
                 class="text-xs font-semibold text-gray-500 uppercase tracking-wide">
            {m.admin_settings_email_rate_per_minute()}
          </label>
          <p class="text-xs text-gray-400 -mt-0.5">
            {m.admin_settings_email_rate_per_minute_hint()}
          </p>
    ...
    {#if pwaSetting}
      <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-4">
        <div class="flex items-center justify-between gap-4">
          <div>
            <p class="text-sm font-semibold text-gray-900">{m.admin_settings_pwa_heading()}</p>
            {#if SETTING_DESCS[pwaSetting.key] ?? pwaSetting.description}
              <p class="text-xs text-gray-400 mt-0.5">{SETTING_DESCS[pwaSetting.key] ?? pwaSetting.description}</p>
            {/if}
          </div>
          <button type="button"
                  onclick={() => (pwaOn = !pwaOn)}
                  class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent
                         transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2
                         {pwaOn ? 'bg-green-500' : 'bg-gray-200'}"
                  role="switch"
                  aria-checked={pwaOn}>
            <span class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform
                         transition duration-200 {pwaOn ? 'translate-x-5' : 'translate-x-0'}"></span>
          </button>
          <input type="hidden" name="pwa_enabled" value={pwaOn ? 'true' : 'false'} />
        </div>
      </div>
    {/if}
    ...
    {#if pdpTaobaoSetting}
      <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-4">
        <div class="flex items-center justify-between gap-4">
          ...
          <button type="button"
                  onclick={() => (pdpTaobaoOn = !pdpTaobaoOn)}
                  class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent
                         transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2
                         {pdpTaobaoOn ? 'bg-green-500' : 'bg-gray-200'}"
                  role="switch"
                  aria-checked={pdpTaobaoOn}>
            <span class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform
                         transition duration-200 {pdpTaobaoOn ? 'translate-x-5' : 'translate-x-0'}"></span>
          </button>
          <input type="hidden" name="pdp_taobao_layout_enabled" value={pdpTaobaoOn ? 'true' : 'false'} />
        </div>
      </div>
    {/if}
