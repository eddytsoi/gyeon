...
<div class="space-y-6">
  ...
  <div class="rounded-2xl border border-gray-100 bg-white px-5 py-4">
    <button type="button"
            ...
            class="w-full flex items-center justify-between text-xs font-semibold text-gray-500 uppercase tracking-wide cursor-pointer select-none">
      <span>{m.admin_redirects_help_title()}</span>
      <svg class="w-3.5 h-3.5 text-gray-400 transition-transform duration-200 ease-out {helpOpen ? 'rotate-180' : ''}"
