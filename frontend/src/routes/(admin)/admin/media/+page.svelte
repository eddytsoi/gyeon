<script lang="ts">
  ...
  function startDelete(id: string) {
    confirmDelete = new Set(confirmDelete.add(id));
  }

  async function doDelete(id: string) {
    try {
      await adminDeleteMedia(data.token, id);
      media = media.filter((f) => f.id !== id);
    } catch {
      // silently ignore
    } finally {
      confirmDelete.delete(id);
      confirmDelete = new Set(confirmDelete);
    }
  }
          ...
          <div class="aspect-square rounded-xl overflow-hidden relative group bg-gray-100">
            ...
            <div class="absolute inset-0 bg-gray-900/70 opacity-0 group-hover:opacity-100 transition-opacity duration-150 flex flex-col justify-between p-2.5">
              ...
              <div class="flex items-center justify-end gap-1.5">
                ...
                {#if confirmDelete.has(file.id)}
                  <button
                    onclick={() => doDelete(file.id)}
                    class="px-2 py-1 rounded-lg bg-red-500 hover:bg-red-600 text-white text-xs font-medium transition-colors"
                  >
                    Confirm?
                  </button>
                  <button
