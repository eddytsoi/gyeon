<script lang="ts">
  ...
  async function handleFiles(files: File[]) {
    ...
    if (rejected > 0) {
      notify.warning(
        rejected === 1
          ? m.admin_product_edit_add_image_rejected_title_one({ count: rejected })
          : m.admin_product_edit_add_image_rejected_title_many({ count: rejected }),
        m.admin_product_edit_add_image_rejected_body()
      );
    }
    ...
    for (const item of newItems) {
      try {
        ...
        if (data.isNew) {
          ...
          if (!res.ok) throw new Error(m.admin_product_edit_add_image_attach_failed({ status: res.status }));
          attached++;
        }
        item.status = 'success';
      } catch (e) {
        item.status = 'error';
        item.error = e instanceof Error ? e.message : m.admin_product_edit_add_image_upload_failed();
      }
  ...
  async function persistReorder(reordered: typeof images) {
    ...
    try {
      ...
      reorderError = m.admin_product_edit_images_reorder_failed();
      images = snapshot;
    } finally {
      reorderSaving = false;
    }
  ...
  <section class="bg-white rounded-2xl border border-gray-100 overflow-hidden mb-6">
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
      <h2 class="font-semibold text-gray-900">
        {m.admin_product_edit_images_heading()}
        {#if !data.isNew && reorderSaving}
          <span class="ml-2 text-xs font-normal text-gray-400">{m.admin_product_edit_images_saving_order()}</span>
        {/if}
      </h2>
      <button onclick={() => showAddImage = true}
              class="px-3 py-1.5 bg-gray-900 text-white text-xs font-medium rounded-lg
                     hover:bg-gray-700 transition-colors">
        {m.admin_product_edit_images_add()}
      </button>
    </div>
    ...
    {#if data.isNew}
      {#if pendingImages.length === 0}
        <div class="px-6 py-8 text-center text-gray-400 text-sm">{m.admin_product_edit_images_empty()}</div>
      {:else}
            ...
            <div class="aspect-square rounded-xl overflow-hidden relative group bg-gray-100">
              ...
              {#if pi.is_primary}
                <span class="absolute top-2 left-2 p-1.5 rounded-lg bg-amber-400/90 text-white"
                      title={m.admin_product_edit_images_tip_primary()} aria-label={m.admin_product_edit_images_aria_primary()}>
              ...
              <div class="absolute inset-0 bg-gray-900/70 opacity-0 group-hover:opacity-100 transition-opacity duration-150 flex items-end justify-center p-2.5">
                <div class="flex items-center justify-center gap-1.5">
                  {#if !pi.is_primary}
                    <button type="button"
                            title={m.admin_product_edit_images_tip_set_primary()} aria-label={m.admin_product_edit_images_aria_set_primary()}
                  ...
                  {/if}
                  <button type="button"
                          title={m.admin_product_edit_images_tip_remove()} aria-label={m.admin_product_edit_images_aria_remove()}
                          onclick={() => {
            ...
            >
              ...
              <div class="absolute inset-0 bg-gray-900/70 opacity-0 group-hover:opacity-100 transition-opacity duration-150 flex items-end justify-center p-2.5">
                <div class="flex items-center justify-center gap-1.5">
                  {#if !image.is_primary}
                    <form method="POST" action="?/setPrimary"
                          use:enhance={() => async ({ result, update }) => {
                            showResult(result, m.admin_product_edit_images_primary_set_success(), m.admin_product_edit_images_primary_set_failure());
                            await update();
                          }}>
                  ...
                  {/if}
                  <button type="button"
                          title={m.admin_product_edit_images_tip_delete()} aria-label={m.admin_product_edit_images_aria_delete()}
                          class="p-1.5 rounded-lg bg-white/10 hover:bg-red-500/80 transition-colors text-white"
...
{#if showAddImage}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-2xl">
      <h3 class="font-semibold text-gray-900 mb-4">{m.admin_product_edit_add_image_title()}</h3>
      ...
      <div class="flex gap-1 mb-5 border-b border-gray-100">
        <button type="button" onclick={() => addImageTab = 'upload'}
          ...
          {m.admin_product_edit_add_image_tab_upload()}
        </button>
        <button type="button" onclick={() => addImageTab = 'library'}
                class="px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors
                       {addImageTab === 'library'
                         ? 'border-gray-900 text-gray-900'
                         : 'border-transparent text-gray-400 hover:text-gray-700'}">
          {m.admin_product_edit_add_image_tab_library()}
        </button>
      </div>
      ...
      {#if addImageTab === 'upload'}
        ...
        <button type="button"
          ...
          <p class="text-sm font-medium text-gray-700">{m.admin_product_edit_add_image_dropzone()}</p>
          <p class="text-xs text-gray-400">{m.admin_product_edit_add_image_accepted()}</p>
        </button>
                ...
                <div class="flex-1 min-w-0">
                  ...
                  {:else}
                    <p class="text-xs text-gray-400">
                      {m.admin_product_edit_add_image_size_kb({ kb: (f.file.size / 1024).toFixed(0) })}
                    </p>
                  {/if}
                </div>
        ...
        <div class="flex gap-3 mt-5">
          <button type="button" onclick={resetAddImageModal}
                  class="flex-1 py-2.5 border border-gray-200 text-gray-600 text-sm rounded-xl
                         hover:border-gray-400 transition-colors">
            {m.admin_product_edit_add_image_done()}
          </button>
        </div>
      {:else}
        ...
        <form method="POST" action="?/addImage"
              use:enhance={({ formData, cancel }) => {
                ...
                attachingImage = true;
                return async ({ result, update }) => {
                  showResult(result, m.admin_product_edit_add_image_added_success(), m.admin_product_edit_add_image_added_failure());
                  await update();
          ...
          {#if imageMedia.length === 0}
            <p class="text-sm text-gray-400 py-6 text-center">{m.admin_product_edit_add_image_no_media()}</p>
          {:else}
          ...
          <div class="flex items-center gap-4 mb-5">
            <div class="flex flex-col gap-1.5">
              <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide">{m.admin_product_edit_add_image_label_primary()}</label>
              <select name="is_primary"
                      class="border border-gray-200 rounded-xl px-3 py-2 text-sm
                             focus:outline-none focus:ring-2 focus:ring-gray-900">
                <option value="false">{m.admin_product_edit_add_image_primary_no()}</option>
                <option value="true">{m.admin_product_edit_add_image_primary_yes()}</option>
              </select>
            </div>
            <div class="flex-1 flex flex-col gap-1.5">
              <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide">{m.admin_product_edit_add_image_label_alt()}</label>
              <input name="alt_text" placeholder={m.admin_product_edit_add_image_alt_placeholder()}
          ...
          <div class="flex gap-3">
            <SaveButton loading={attachingImage} disabled={!addImageSelectedId}
              ...
              {m.admin_product_edit_add_image_submit()}
            </SaveButton>
            <button type="button" onclick={resetAddImageModal}
                    class="flex-1 py-2.5 border border-gray-200 text-gray-600 text-sm rounded-xl
                           hover:border-gray-400 transition-colors">
              {m.admin_product_edit_add_image_cancel()}
            </button>
          </div>
        </form>
      {/if}
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
      <div class="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 class="font-semibold text-gray-900">{m.admin_product_edit_delete_image_title()}</h3>
          <p class="text-sm text-gray-500">{m.admin_product_edit_delete_image_irreversible()}</p>
        </div>
      </div>
      <form method="POST" action="?/deleteImage"
            use:enhance={() => {
              ...
              deletingImage = true;
              return async ({ result, update }) => {
                showResult(result, m.admin_product_edit_delete_image_success(), m.admin_product_edit_delete_image_failure());
        ...
        <div class="flex gap-2 justify-end">
          <button type="button"
            ...
            {m.admin_product_edit_delete_image_cancel()}
          </button>
          <SaveButton loading={deletingImage}
            ...
            {m.admin_product_edit_delete_image_confirm()}
          </SaveButton>
        </div>
      </form>
    </div>
